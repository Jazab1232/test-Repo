...
export default function ProjectCard({ projectName, priority, dueDate, companyName, width, id, IsComplete }) {
    const { setSelectedTaskId, selectedTaskId, tasks, projects, setProjects, teamMembers } = useContext(AppContext);
    ...
    return (
        <div  className='projectCard' style={{ width: width }}>
            <div className="projectCardHead">
                ...
                <i onClick={toggleMenu} className="fa-solid fa-ellipsis" style={{ fontSize: '16px', cursor: 'pointer' }}></i>
            </div>
            <div className="projectStatus">
                <p style={{ margin: '0', textTransform: 'uppercase' }}>{IsComplete}</p>
            ...
            <div className="cardMenu" style={{ display: menuVisible ? 'flex' : 'none' }}>
                <Link to={`/project-detail?id=${id}`} style={{ color: 'blue' }} className="cardMenuEdit">View</Link>
                {role == 'admin' && (
